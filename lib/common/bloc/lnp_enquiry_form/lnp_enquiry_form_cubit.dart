...
class LnpEnquiryFormCubit extends Cubit<LnpEnquiryFormState> {
  ...
  void validatePersonalForm(LNPEnquiryFormParams param) {
    if (param.mobileNumber.trim().length != 10 || int.tryParse(param.mobileNumber.trim())==null) {
    ...
    } else if (param.pinCode.trim().isEmpty || int.tryParse(param.pinCode.trim())==null) {
    ...
    else if (param.totalCableTvSubscriberCount.isEmpty || int.tryParse(param.totalCableTvSubscriberCount.trim())==null) {
      emit(
        LnpFormValidationError(
          errorMessage: 'Enter Total No of existing CableTV Subscriber',
        ),
      );
    } else if (param.totalInternetSubscriberCount.isEmpty || int.tryParse(param.totalInternetSubscriberCount.trim())==null) {
      emit(
        LnpFormValidationError(
          errorMessage: 'Enter Total No of existing Internet Subscriber',
        ),
      );
    } else if (param.totalFiberCount.isEmpty || int.tryParse(param.totalFiberCount.trim())==null) {
      emit(
        LnpFormValidationError(
